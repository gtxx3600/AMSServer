...
public class AMSServer {
  ...
  Map<String,ObjectOutputStream> firstSockets;
  // 保存当前的连接信息
  Set connections; 
  // 支持的最大并发连接数
  int maxConnections; 
   ...
   * 指定日志消息输出流和最大并发连接数。
   **/
  public AMSServer(OutputStream logStream, int maxConnections) {
    // 初始化各实例变量
    this.setLogStream(logStream);
    this.log("Starting server");
    // 创建一个线程组，所有启动的线程都在该组内
    this.threadGroup = new ThreadGroup(AMSServer.class.getName());
    this.maxConnections = maxConnections;
    this.services = new HashMap();
    this.connections = new HashSet(maxConnections);
    this.firstSockets=new TreeMap<String,ObjectOutputStream>();
  }
...
//      catch(Exception e)
//      {
//        log("SetTimeOut Error in socket :"+client+"error info:"+e.getLocalizedMessage());
//      }
     if(service instanceof MessageServer)
      {
        ((MessageServer)service).transSockets(firstSockets);
      }
    }
    /**
     * 连接的线程体
    ...
    public void run() {
      try {
        ...
        // 调用具体的服务
        if(service instanceof UserLogin)//service.getClass().getCanonicalName().indexOf("UserLogin")>=0)
        {
          ObjectOutputStream output =new ObjectOutputStream(out);
          ObjectInputStream input=new ObjectInputStream(in);
          ((UserLogin)service).transSockets(firstSockets,client);
...
//  }
  ...
  public static class Control implements Service {
     ...
     * @param password
     */
    public Control(AMSServer server, String password) {
      this.server = server;
